...
```
  ...
  浏览器 ── HTTPS ──▶ │ Caddy / Nginx（80/443，自动证书）                  │
                      │                                                  │
                      │  /login       → 127.0.0.1:3081  首页登录页（匿名） │
                      │  /api/auth/*  → 127.0.0.1:3081  登录 / 鉴权探针    │
                      │  其余          → 先 forward_auth 探 3081，通过后： │
                      ...
                      └──────────────────────────────────────────────────┘
                          ...
                          systemd: lma.service（User=dsh）
                                        │
                     SQLite: /var/lib/lma/lma.db（不在仓库目录里）
```

- **访客只看到一道门**：打开 `https://<域名>/` 未登录会被送到 `/login`；登录一次之后
  聊天 UI 与 `/lma/` 仪表盘通用同一条会话 Cookie，**不会要求第二次登录**。
- **鉴权由反代 + 插件的 `/api/auth/verify` 分工完成**：反代对 `/login`、`/api/auth/*`、
  `/lma/unsubscribe*` 之外的请求先 `forward_auth` 探一下；插件 200 表示放行，页面导航时
  未登录直接回 `302 → /login?next=…`，反代把非 2xx **原样转给浏览器**。
  ⚠️ 不要在 `forward_auth` 的 `handle_response` 里写 `redir`：适配器会把以 `/` 开头的第一个
  参数当成**路径匹配器**（`redir /login?next=… 302` → "匹配 `/login?next=…`、跳转到 `302`"），
  `caddy validate` 照样通过但行为完全不对。
...
- **别再叠一层 Basic Auth**：那会让同事登录两次。统一账号体系与 Basic Auth 二选一。
