...
| `LMA_BASE_URL` | `http://127.0.0.1:3080` | 退订链接域名（部署后改成你的 HTTPS 域名） |
...
- Nginx 反代 + Let's Encrypt HTTPS（登录页强制加密），`LMA_BASE_URL` 设为正式域名
- SQLite 每日备份（`LMA_DB_BACKUP_DIR`）或 systemd timer 复制 `lma-data/lma.db`
