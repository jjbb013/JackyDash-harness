...
- 打开聊天台（本机 `http://127.0.0.1:3080`；公网 `https://<域名>/`），侧边栏顶部点击 **LMA 推广**
  图标，主区即嵌入仪表盘。iframe 地址按部署形态推断（`src/client/dashboard-url.ts`）：
  **本机回环** → `http://127.0.0.1:3081/`（仪表盘在另一个端口）；**其余** → 同源 `/lma/`。
  ⚠️ 别写死 `http://127.0.0.1:3081/`：公网访客的浏览器会去连**自己的本机** 3081，面板一片空白，
  而且本机怎么测都测不出来。
- 也可直接访问仪表盘（本机 `http://127.0.0.1:3081/`；公网 `https://<域名>/lma/`），
  七个 tab：总览 / 审核队列 / 供应商 / 发送队列 / 退订名单 / 配置（仅 admin）/ 人员管理（仅 admin）
- 全部接口都要登录（会话 Cookie）；权限按角色判定，后端强制（见 §5.6），前端隐藏只是体验
- 服务默认只绑 127.0.0.1，公网一律经反向代理
